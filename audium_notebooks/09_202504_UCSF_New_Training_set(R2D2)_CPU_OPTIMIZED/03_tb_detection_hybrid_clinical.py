...
# Removed KNeighborsClassifier - replaced with WHO-optimized MLP
...
def optimize_who_threshold(y_true, y_scores):
    """Aggressive threshold optimization prioritizing sensitivity for WHO compliance"""
    if y_scores is None or len(np.unique(y_true)) < 2:
        return 0.5, 0, 0, False
    
    thresholds = np.linspace(0, 1, 2000)  # Higher resolution
    best_threshold = 0.5
    best_sensitivity = 0
    best_specificity = 0
    who_compliant = False
    
    # Track best WHO-compliant threshold
    best_who_threshold = None
    best_who_sens = 0
    best_who_spec = 0
    ...
    for threshold in thresholds:
        ...
        # Priority 1: WHO compliance with relaxed specificity (sens ≥90%, spec ≥60%)
        if sens >= 0.9 and spec >= 0.6:
            if sens > best_who_sens or (sens == best_who_sens and spec > best_who_spec):
                best_who_threshold = threshold
                best_who_sens = sens
                best_who_spec = spec
                who_compliant = spec >= 0.7  # True compliance still requires ≥70% spec
        
        # Priority 2: Maximum sensitivity with minimal specificity constraint (≥40%)
        if spec >= 0.4 and sens > best_sensitivity:
            best_threshold = threshold
            best_sensitivity = sens
            best_specificity = spec
    
    # Use WHO-compliant threshold if found, otherwise use best sensitivity
    if best_who_threshold is not None:
        return best_who_threshold, best_who_sens, best_who_spec, who_compliant
    
    return best_threshold, best_sensitivity, best_specificity, who_compliant

class WHOLoss(nn.Module):
    """Aggressive WHO-optimized loss function with extreme sensitivity focus"""
    
    def __init__(self, fn_weight=10.0, fp_weight=1.0):
        super().__init__()
        self.fn_weight = fn_weight  # Extreme penalty for missing TB cases
        self.fp_weight = fp_weight  # Light penalty for false alarms
    
    def forward(self, predictions, targets):
        # Asymmetric binary cross-entropy loss with focal loss component
        # Standard asymmetric BCE
        bce_loss = -targets * self.fn_weight * torch.log(predictions + 1e-8) - \
                   (1 - targets) * self.fp_weight * torch.log(1 - predictions + 1e-8)
        
        # Add focal loss component to focus on hard examples
        alpha = 0.75  # Weight for positive class
        gamma = 2.0   # Focusing parameter
        
        # Focal loss for hard negatives (missed TB cases)
...
class PyTorchMLPWrapper:
    ...
    def __init__(self, hidden_dims=[100, 50], epochs=100, lr=0.001, batch_size=32, device='cpu', loss_fn=None):
...
def get_gpu_optimized_models(config):
    ...
    models.update({
        ...
        "Logistic Regression": LogisticRegression(
            ...
            class_weight={0: 1, 1: 8}, random_state=config.random_state, max_iter=1000
        ),
        "WHO-MLP": PyTorchMLPWrapper(
            hidden_dims=[100, 50], epochs=150, lr=0.001,
            device=config.pytorch_device, loss_fn=WHOLoss(fn_weight=10.0, fp_weight=1.0)
        )
    })
    ...
    if XGBOOST_AVAILABLE:
        ...
        models.update({
            "Random Forest": RandomForestClassifier(
                n_estimators=100, max_depth=10, class_weight={0: 1, 1: 8},
            ...
            "Decision Tree": DecisionTreeClassifier(
                max_depth=10, class_weight={0: 1, 1: 8}, random_state=config.random_state
            )
        })
...
def evaluate_model_single_fold(X_train, X_test, y_train, y_test, model, model_name, config):
    ...
    try:
        ...
        # Threshold optimization for WHO compliance
        optimal_threshold = 0.5
        optimal_sensitivity = sensitivity
        optimal_specificity = specificity
        optimal_who_compliant = False
        ...
        who_compliant = optimal_who_compliant
...
def evaluate_models_cross_validation(X, y, config):
    ...
    def evaluate_model_cv(model_name, model):
        ...
        if fold_results:
            ...
            mean_results['who_compliant_folds'] = who_compliant_folds
...
def create_cv_fold_variance_plots(results, config):
    ...
    # Plot 4: WHO Score variance across folds (using optimized metrics)
    ax = axes[1, 1]
    for i, result in enumerate(results):
        fold_who_scores = []
        for fold in result['fold_results']:
            # Use the actual WHO score from fold evaluation (already optimized)
            who_score = fold.get('who_score', 0)
            fold_who_scores.append(who_score)
        
        if fold_who_scores:  # Only plot if we have data
...
def create_who_compliance_analysis(results, config):
    ...
    # Plot 1: WHO Compliance Status (using optimized thresholds)
    ax = axes[0]
    who_compliant = [r.get('optimal_who_compliant', r.get('who_compliant', False)) for r in results]
    colors = ['green' if compliant else 'red' for compliant in who_compliant]
    ...
    # Plot 2: WHO Score Distribution (using optimized metrics)
    ax = axes[1]
    ...
    # Plot 3: Sensitivity vs Specificity with WHO targets (optimized)
    ax = axes[2]
...
def create_cv_who_compliance_analysis(results, config):
    ...
    # Plot 1: WHO Compliance Rate by Model (using optimized thresholds)
    ax = axes[0]
    compliance_rates = [r.get('who_compliant_folds', 0) / r['total_folds'] for r in results]
    colors = ['green' if rate >= 0.8 else 'orange' if rate >= 0.5 else 'red' for rate in compliance_rates]
